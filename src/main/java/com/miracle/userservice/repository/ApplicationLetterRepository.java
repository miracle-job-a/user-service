...
public interface ApplicationLetterRepository extends JpaRepository<ApplicationLetter, Long> {
    ...
    @Query("""
           SELECT new com.miracle.userservice.dto.response.ApplicantListResponseDto(al.id, al.user.id, al.resumeTitle, al.userEmail, al.userName, al.user.address, al.submitDate)
