...
public class ResumePostRequestDto {
    ...
    @Schema(description = "이력서 제목", required = true, example = "Software Engineer")
    private final String title;

    @Size(max = 20, message = ValidationDefaultMsgUtil.ResumePost.EDUCATION)
    @Schema(description = "학력 정보", example = "Bachelor's Degree")
    ...
    @Schema(description = "사진 URL", example = "photo-url")
    private final String photo;

    @PositiveOrZero(message = ValidationDefaultMsgUtil.ResumePost.CAREER)
    @Schema(description = "경력 연수", example = "3")
    private final int career;

    @Schema(description = "이력서 공개 여부", example = "true")
    private final boolean open;

    @NotNull(message = ValidationDefaultMsgUtil.ResumePost.COLLECTION)
    @ArraySchema(schema = @Schema(description = "기술 스택 ID 목록", required = true))
    private final Set<Long> stackIdSet;

    @NotNull(message = ValidationDefaultMsgUtil.ResumePost.COLLECTION)
    @ArraySchema(schema = @Schema(description = "직무 ID 목록", required = true))
    private final Set<Long> jobIdSet;

    @NotNull(message = ValidationDefaultMsgUtil.ResumePost.COLLECTION)
    @ArraySchema(schema = @Schema(description = "경력 상세 목록", required = true))
    private final List<String> careerDetailList;

    @NotNull(message = ValidationDefaultMsgUtil.ResumePost.COLLECTION)
    @ArraySchema(schema = @Schema(description = "프로젝트 목록", required = true))
    private final List<String> projectList;

    @NotNull(message = ValidationDefaultMsgUtil.ResumePost.COLLECTION)
    @ArraySchema(schema = @Schema(description = "기타 스킬 목록", required = true))
